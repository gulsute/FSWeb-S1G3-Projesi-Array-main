...
function dizi25Cesitmi (diziSayisi) {
  if (diziSayisi.length === 25) {
  return true;
} else {
  return false;
}
...
function cesitEkle (yeniOrjTatlar, yeniLezzet) {
  yeniOrjTatlar.unshift(yeniLezzet);
  return yeniOrjTatlar
}
...
function sonCesitiKaldir(yeniOrjTatlar){
yeniOrjTatlar.pop()
return yeniOrjTatlar}
...
function indekstekiCesitiGetir(yeniOrjTatlar, sira){
  return yeniOrjTatlar[sira]
}
...
function ismeGoreCesitCikar(yeniOrjTatlar, aroma){
  let index = yeniOrjTatlar.indexOf (aroma);
  yeniOrjTatlar.splice(index,1);
  return yeniOrjTatlar}
...
function ismeGoreFiltrele(yeniOrjTatlar, filtreDegeri){
  
  const filtreliOrjTatlar = [];

  for (let i=0; i < yeniOrjTatlar.length ; i++){

    if ( yeniOrjTatlar[i].includes(filtreDegeri)){
        filtreliOrjTatlar.push(yeniOrjTatlar[i])
  } }
    return filtreliOrjTatlar }
